...
static uint8_t is_better_lighter(object *current, object *old) {
    ...
    if (current->type == SKILL && (current->subtype == SK_CLAWING || old->subtype == SK_FLAME_TOUCH))
        return 1;
    // If old tool is a lighter and has higher or equal speed than a lighter as the new tool, then it is the better tool.
    if (old->type == LIGHTER && (current->type != LIGHTER || old->speed >= current->speed))
        return 0;
    // And if the new tool has those same criteria, then it is better.
    if (current->type == LIGHTER && (old->type != LIGHTER || current->speed > old->speed))
        return 1;
    // For spells, either take the lower mana cost between spells or supercede a skill or skill tool.
    if (old->type == SPELL && (current->type != SPELL || old->stats.sp + old->stats.grace <= current->stats.sp + current->stats.grace))
        return 0;
    if (current->type == SPELL && (old->type != SPELL || current->stats.sp + current->stats.grace < old->stats.sp + old->stats.grace))
        return 1;
